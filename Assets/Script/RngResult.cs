...
public class RngResult : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        goldText.GetComponent<Text>().text = RngManager.gold.ToString("0") + "/5";
    ...
    {
        if (RngManager.clearTime <= 60 && RngManager.gold >= 5 && RngManager.hpScore >= 3)     // 클리어타임이 60초 이하이고 골드가 5이상이고 체력이 3이상이면 S
            gradeText.GetComponent<Text>().text = "S";
        else if (RngManager.clearTime <= 90 && RngManager.gold >= 5 && RngManager.hpScore >= 2)     // 클리어타임이 90초 이하이고 골드가 5이상이고 체력이 2이상이면 A
            gradeText.GetComponent<Text>().text = "A";
        else if (RngManager.clearTime <= 120 && RngManager.gold >= 3 && RngManager.hpScore >= 2)      // 클리어타임이 120초 이하이고 골드가 3이상이고 체력이 2이상이면 B
            gradeText.GetComponent<Text>().text = "B";
        else if (RngManager.clearTime <= 150 && RngManager.gold >= 2 && RngManager.hpScore >= 1)     // 클리어타임이 150초 이하이고 골드가 2이상이고 체력이 1이상이면 C
